...
Version: %s
Architecture: %s
Maintainer: xdesk <info@rustdesk.com>
...
def build_flutter_deb(version, features):
    ...
    system2('mkdir -p tmpdeb/usr/share/xdesk')
    system2('mkdir -p tmpdeb/etc/xdesk/')
    system2('mkdir -p tmpdeb/etc/pam.d/')
    system2('mkdir -p tmpdeb/usr/share/xdesk/files/systemd/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/256x256/apps/')
    system2('mkdir -p tmpdeb/usr/share/icons/hicolor/scalable/apps/')
    system2('mkdir -p tmpdeb/usr/share/applications/')
    system2('mkdir -p tmpdeb/usr/share/polkit-1/actions')
    system2('rm tmpdeb/usr/bin/xdesk || true')
    system2(
        f'cp -r {flutter_build_dir}/* tmpdeb/usr/share/xdesk/')
    system2(
        'cp ../res/xdesk.service tmpdeb/usr/share/xdesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/xdesk.png')
    system2(
        'cp ../res/scalable.svg tmpdeb/usr/share/icons/hicolor/scalable/apps/xdesk.svg')
    system2(
        'cp ../res/xdesk.desktop tmpdeb/usr/share/applications/xdesk.desktop')
    system2(
        'cp ../res/xdesk-link.desktop tmpdeb/usr/share/applications/xdesk-link.desktop')
    system2(
        'cp ../res/startwm.sh tmpdeb/etc/xdesk/')
    system2(
        'cp ../res/xorg.conf tmpdeb/etc/xdesk/')
    system2(
        'cp ../res/pam.d/xdesk.debian tmpdeb/etc/pam.d/xdesk')
    system2(
        "echo \"#!/bin/sh\" >> tmpdeb/usr/share/xdesk/files/polkit && chmod a+x tmpdeb/usr/share/xdesk/files/polkit")
...
def build_deb_from_folder(version, binary_folder):
    ...
    system2('rm tmpdeb/usr/bin/xdesk || true')
    system2(
        f'cp -r ../{binary_folder}/* tmpdeb/usr/share/xdesk/')
    system2(
        'cp ../res/xdesk.service tmpdeb/usr/share/xdesk/files/systemd/')
    system2(
        'cp ../res/128x128@2x.png tmpdeb/usr/share/icons/hicolor/256x256/apps/xdesk.png')
...
def build_flutter_windows(version, features, skip_portable_pack):
    ...
    system2(
        f'python3 ./generate.py -f ../../{flutter_build_dir_2} -o . -e ../../{flutter_build_dir_2}/xdesk.exe')
    os.chdir('../..')
    if os.path.exists('./xdesk_portable.exe'):
        ...
        os.rename('./target/release/rustdesk-portable-packer.exe',
                  './xdesk_portable.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/xdesk_portable.exe')
    os.rename('./xdesk_portable.exe', f'./xdesk-{version}-install.exe')
    print(
        f'output location: {os.path.abspath(os.curdir)}/xdesk-{version}-install.exe')
...
def main():
    ...
    if windows:
        ...
        if flutter:
            ...
            system2('cargo build --release --features ' + features)
            system2('git checkout src/ui/common.tis')
            system2('strip target/release/xdesk')
            system2('ln -s res/pacman_install && ln -s res/PKGBUILD')
            system2('HBB=`pwd` makepkg -f')
        system2('mv xdesk-%s-0-x86_64.pkg.tar.zst xdesk-%s-manjaro-arch.pkg.tar.zst' % (
            version, version))
